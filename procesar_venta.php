...
$stmt = pg_prepare($conexion, $sql_venta_encabezado);

if ($stmt) {
    // Tipos de parámetros: s (fecha), i (idTrab), d (subtotal), d (IVA), d (total)
    pg_stmt_bind_param($stmt, "siddd", 
        $fecha_venta, 
        $id_trabajador, 
        $subtotal_final, 
        $iva_final, 
    ...
    if (pg_stmt_execute($stmt)) {
        $id_venta_insertada = pg_insert_id($conexion);
        pg_stmt_close($stmt);
        ...
        $sql_venta_detalle = "INSERT INTO ventasdet (idVenta, idProd, cantidad, precio) 
                              VALUES (?, ?, ?, ?)";
        $stmt_detalle = pg_prepare($conexion, $sql_venta_detalle);
        ...
        if ($stmt_detalle) {
            foreach ($productos_vendidos as $producto) {
                ...
                if ($idProd > 0 && $cantidad > 0) {
                    ...
                    pg_stmt_bind_param($stmt_detalle, "iiid", 
                        $id_venta_insertada, 
                        $idProd, 
                        $cantidad, 
                        $precio_unitario
                    ...
                    if (!pg_stmt_execute($stmt_detalle)) {
                        $todo_ok = false;
                        $error_msg = "Error al insertar el detalle. Producto ID: $idProd. Error: " . pg_stmt_error($stmt_detalle);
                        break; 
                    }
                }
            }
            pg_stmt_close($stmt_detalle);
        } else {
            $todo_ok = false;
            $error_msg = "Error al preparar la sentencia de detalle: " . pg_error($conexion);
        }
    } else {
        $todo_ok = false;
        $error_msg = "Error al ejecutar la sentencia de encabezado: " . pg_stmt_error($stmt);
    }
} else {
    $todo_ok = false;
    $error_msg = "Error al preparar la sentencia de encabezado: " . pg_error($conexion);
}
...
pg_autocommit($conexion, TRUE); // Restaurar autocommit
pg_close($conexion);
